function Setup() {
    ...
    } else if (App.VERSION == "2.1.0") {
        // COLOR
        Color.BRICK = Color.value(0x884444);
        Color.WATER = Color.value(0x4488AA);
        // DRAW
        App.draw = Draw().create(Width(), Height()).full().add();
        // BRIDGE
        Script().preload(Resources(Path(["Source", App.VERSION, "Bridge"]), "bridge").files(["Load", "Value", "About", "Menu", "Main"])).response(function(total, count) {
